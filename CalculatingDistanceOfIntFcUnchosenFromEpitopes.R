...
intfc_unchosen_B<- c(72,174,200,245,250,278,279,283,321,338,340,341,342)
intfc_unchosen_B<-paste(intfc_unchosen_B, "B")
intfc_unchosen_F<- c(59,309,310,359,383,385,386,387,388,391)
intfc_unchosen_F<-paste(intfc_unchosen_F, "F")
...
dB <- data.frame(matrix(nrow = length(c(intfc_unchosen_B,intfc_unchosen_F)), ncol = length(c(epitopeB,epitopeF))))
rownames(dB) <- c(intfc_unchosen_B,intfc_unchosen_F)
...
for (i in c(intfc_unchosen_B, intfc_unchosen_F)) {
  for (j in c(epitopeB,epitopeF)) {
...
openxlsx::write.xlsx(dB, "distancefromInterfaceUnchosenAndEpitopes.xlsx",rowNames = TRUE)
